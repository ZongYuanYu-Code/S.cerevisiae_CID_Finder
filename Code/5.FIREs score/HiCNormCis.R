...
option_list = list(
  ...
  make_option(c("-n", "--negative_binomial"), action = "store_true", default =FALSE,
...
genomic_features <- read.table(genomic_features_in,head=F, sep="\t", colClasses=c("factor",rep("numeric",4)))
colnames(genomic_features) <- c('chr', 'bin1', 'len', 'gc', 'map')
	...
	tryCatch(
		{
			...
			#message(paste("feture",genomic_features_chr,sep=" "))
			
			map_chr <- map[which(map$chr==chr),]
			#select bins with confident genomic features to build model
			conf_bins <- which(genomic_features_chr$len>=min_len & genomic_features_chr$gc>=min_gc & genomic_features_chr$map>=min_map) 
			#message(conf_bins)
			
			ind1 <- match(map_chr$bin1,genomic_features_chr$bin1)
			#message(paste("aaaaa",ind1,sep=" "))
			#ind2 <- match(map_chr$bin2,genomic_features_chr$bin1)

#			map_chr$len <- log(genomic_features_chr$len[ind1])
#			map_chr$gc <- log(genomic_features_chr$gc[ind1])
#			map_chr$map <- log(genomic_features_chr$map[ind1])
			map_chr$len <- genomic_features_chr$len[ind1]
                        map_chr$gc <- genomic_features_chr$gc[ind1]
                        map_chr$map <- genomic_features_chr$map[ind1]

			res <- map_chr

    		
			map_fit <- map_chr[!is.na(map_chr$len) & map_chr$raw >= min_cov & ind1 %in% conf_bins ,]
			...
			message(opt$negative_binomial)
			#message(map_fit$len)
			#message(map_fit$gc)
			message(map_fit$map)
			message("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
...
#final
write.table(final[,c(1,2,7)], file=out, row.names=F, col.names=F, sep="\t", quote=F, na = "NaN")
